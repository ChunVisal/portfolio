...
// config
const SECTIONS = ["Hero","About","Projects","TechStack","Footer"];
...
<template>
  <div class="home-root">
    ...
    <!-- SIDE NAV -->
    <div class="side-nav fixed right-4 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-4">
      <div v-for="(s,i) in SECTIONS" :key="i" class="flex items-center gap-3 cursor-pointer" @click="goToSection(i)">
        <div :class="['w-2 h-2 rounded-full transition-all duration-300', Math.floor(targetProgress) === i ? 'bg-blue-300 scale-125 ring-2 ring-blue-200' : 'bg-gray-400 hover:scale-110']"></div>
        <span class="text-sm font-medium whitespace-nowrap transition-all duration-300" :class="Math.floor(targetProgress) === i ? 'text-white' : 'text-gray-400'" :style="{opacity: Math.floor(targetProgress) === i ? 1 : 0.45}">
          {{ i === 0 ? 'Home' : i === 1 ? 'About' : i === 2 ? 'Projects' : i === 3 ? 'Tech Stack' : 'Contact' }}
        </span>
      </div>
    </div>
    ...
    <div ref="cameraRef" class="viewport fixed inset-0 z-10 overflow-hidden">
      <!-- SECTION 0 - HERO -->
      <section class="page-section absolute inset-0" :style="{ opacity: sectionOpacity(0), pointerEvents: Math.floor(progress) === 0 ? 'auto' : 'none' }">
        <Hero />
      </section>
